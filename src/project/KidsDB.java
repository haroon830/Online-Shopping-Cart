...
public class KidsDB {
    ...
    public static void insertIntoKidsDB(String brand, String model, int price, int qty, String description, String imagePath) throws IOException{
        try{  
    ...
    public static ArrayList<ProductList> homePageContent(){
        ...
        try{  
          ...
          Connection con=DriverManager.getConnection( "jdbc:oracle:thin:@localhost:1521:orcl","hr","oracle");
            Statement ps = con.createStatement();
            ResultSet rs = ps.executeQuery("SELECT mbrand, mmodel, mprice,mquantity, mdescription, mphoto FROM kids ORDER BY id ");
            ProductList pl;
            while(rs.next()){
                pl = new ProductList(rs.getString("mbrand"),rs.getString("mmodel"),
                        rs.getInt("mprice"),rs.getInt("mquantity"),rs.getString("mdescription"),
